...
async def get_by_id(task_id : UUID, db : AsyncSession = Depends(get_db)):
   return await get_one_task(db, task_id) 

@router.put("/{task_id}", response_model=TaskSchema, status_code=status.HTTP_200_OK)
async def update(task_id : UUID, task_data : TaskUpdate, db : AsyncSession = Depends(get_db)):
    return await change_task(db, task_id, task_data)

@router.delete("/{task_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete(task_id : UUID, db : AsyncSession = Depends(get_db)):
